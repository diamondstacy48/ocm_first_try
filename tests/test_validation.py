...
#Заваливается при ошибке "Используйте только русские буквы и цифры"
# @allure.feature('Валидация полей на втором шаге регистрации')
# @allure.severity(allure.severity_level.CRITICAL)
...
# тест заваливается на 2 последних значениях из-за перекрытия сообщения об ошибке дадатой..
# @allure.feature('Валидация полей на втором шаге регистрации')
# @allure.severity(allure.severity_level.CRITICAL)
...
#                 expect(second_step_reg.error_wrong_pass_code).to_have_text('Значение «Код подразделения» неверно.».')
#                 error_message = second_step_reg.error_wrong_pass_code.text_content()
#             else:
#                 expect(second_step_reg.error_empty_pass_code).to_be_attached()
